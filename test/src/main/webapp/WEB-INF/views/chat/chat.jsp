...
function print(user, txt) {
	...
	//Server에서 받아와서 가공작업 ([2#]2#user : txt형태 가공)
	if(user.indexOf('#') != -1){
		let idx = txt.indexOf(':');
		user = txt.substr(2, idx-2);
		let text = txt.substr(idx+1);
		txt = text;
		
		temp += '<div style="margin-bottom:3px; background-color : #FFFF99; border-radius : 10px; padding : 5px; text-align : left; ">';
		temp += '[' + user + '] ';
		temp += txt;
		...
		temp += '<div style="margin-bottom:3px; background-color : #99CCFF; border-radius : 10px; padding : 5px; text-align : right;">';
		temp += '[' + user + '] ';
		temp += txt;
		temp += ' <span style="font-size:11px;color:#777;">' + new Date().toLocaleTimeString() + '</span>';
		temp += '</div>';
